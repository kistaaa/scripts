...
public class BombBehaviour : MonoBehaviour
{
    ...
    {
        //Explosão central
        GameObject effect = PhotonNetwork.Instantiate(explosionEffect.name, transform.position, Quaternion.identity);
        Destroy(effect, 2f);
    }
    ...
    void Explosion(GameObject effect, Vector3 direction)
    {
        for (int i = 1; i < explosionSize; i++)
        {
            ...
            GameObject fx = PhotonNetwork.Instantiate(effect.name, explosionPos, Quaternion.identity);
            Destroy(fx, 2f);
        }
    }
